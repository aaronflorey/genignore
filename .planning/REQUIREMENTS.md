...
- [x] **CMD-01**: User can run `genignore detect` to auto-detect providers from project and environment signals
- [x] **CMD-02**: User can run `genignore detect --include <keys>` to force-add valid providers before generation
- [x] **CMD-03**: User can run `genignore detect --exclude <keys>` to remove providers before generation
- [x] **CMD-04**: User sees a hard error when `detect` resolves to an empty final provider set
- [x] **CMD-05**: User can run `genignore add <keys...>` to append only missing valid providers to the managed set
